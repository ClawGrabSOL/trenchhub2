...
export default function ExplorePage() {
  ...
  const fetch_ = useCallback(async (s, cat, srt) => {
    setLoading(true);
    try {
  ...
  useEffect(() => { fetch_(search, category, sort); }, [sort, category]);

  const handleSearch = (e) => { e.preventDefault(); fetch_(search, category, sort); };
